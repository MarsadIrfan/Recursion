...
DRAW_FRACTAL = 6 # Set to 1 through 11 and run the program

turtle.tracer(5000, 0) # Increase the speed of first argument to speed up the drawing
turtle.hideturtle()
...
def drawFilledSquare(size, depth):
	...
	# Alteranate between white and gray (with black border)
	if depth % 2 == 0:
...
def drawFilledRegularHexagon(size, depth):
	size = int(size)
	...
	# Draw an Hexagon
	turtle.begin_fill()
	for i in range(6): #Draw four lines
		turtle.forward(size)
		turtle.right(60)
	turtle.end_fill()
...
def drawTriangleOutline(size, depth):
	...
	# Move the turtle to the top of the equilateral traingle:
	height = size * math.sqrt(3) / 2
	turtle.penup()
	turtle.left(90) # turn the turtle to face upward
	turtle.forward(height * (2/3)) # Move to the corner
	turtle.right(150) # Turn to face the bottom
	turtle.pendown()
...
def drawFractal(shapeDrawFunction, size, specs, maxDepth=8, depth=0):
	...
	for spec in specs:
		...
		# are multiplied by the size parameter
...
elif DRAW_FRACTAL == 12:
	# Hexagon Snow
	drawFractal(drawFilledRegularHexagon, 200,
	[{'xChange': math.cos(0 * math.pi / 180),
	'yChange': math.sin(0 * math.pi / 180), 'sizeChange': 0.4},
	{'xChange': math.cos(72 * math.pi / 180),
	'yChange': math.sin(72 * math.pi / 180), 'sizeChange': 0.4},
...
elif DRAW_FRACTAL == 13:
	# Spiral Hexagon
	drawFractal(drawFilledRegularHexagon, 600, [{'sizeChange':0.95, 'angleChange':7}], 50)
